...
int main()
{
	...
	if( true ) {
		...
		//
		Net net;
		net << shared_ptr<Layer>(std::make_shared<FullyConnected_Layer>(MNIST_IMG_SZ, 100))
				<< shared_ptr<Layer>(std::make_shared<ReLU_Layer>(100))
				<< shared_ptr<Layer>(std::make_shared<FullyConnected_Layer>(100, 100))
				<< shared_ptr<Layer>(std::make_shared<ReLU_Layer>(100))
				<< shared_ptr<Layer>(std::make_shared<FullyConnected_Layer>(100, 10))
				<< shared_ptr<Layer>(std::make_shared<SoftMax_Layer>(10));
		//	全データ（６万）で1回学習（１エポック）、ミニバッチ（バッチサイズ：100）
		const int BATCH_SIZE = 100;
		int n_correct = 0;
